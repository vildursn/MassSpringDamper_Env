...
class MassSpringDamperFullEnv(gym.Env):
    metadata = {
    ...
    def __init__(self): #, spring_stiffness, damper_factor, mass, max_force4
        ...
        obs_high = np.array([self.x_treshold,np.finfo(np.float32).max,self.x_treshold,np.finfo(np.float32).max])#,self.x_treshold,np.finfo(np.float32).max])
    ...
    def step(self, action):
        ...
        self.state = (x[0],x_dot[0],x_goal,x_dot_goal)
        done = False
        reward = -np.abs(x - x_goal)**2 - 0.1*np.abs(action) - 1
        if (np.abs(x- x_goal) < 0.001) and (np.abs(x_dot - x_dot_goal)< 0.001):# and (np.abs(x_dot -x_dot_goal)< 0.01):
            #print("YAY! Managed to arrive at righ.")
            reward = 1
            if (np.abs(x- x_goal) < 0.0001) and(np.abs(x_dot -x_dot_goal)< 0.0001):
                reward = 10
                print("yay")
                done = True
