...
class MassSpringDamperFullEnv_2(gym.Env):
    ...
    def reset(self):
        state = [np.random.uniform(low = -self.x_treshold/2.0, high = self.x_treshold/2.0),np.random.uniform(low = -0.01, high = 0.01)]
        goal_state = [np.random.rand()*2*(self.x_treshold-0.5)-(self.x_treshold-0.5),0.00]
        self.goal_state =(goal_state[0], goal_state[1])
        self.state = (state[0],state[1],(goal_state[0]-state[0]))
        while (np.abs(self.state[0] -self.goal_state[0])<1):
            state = [np.random.uniform(low = -self.x_treshold/2.0, high = self.x_treshold/2.0),np.random.uniform(low = -0.01, high = 0.01)]
            goal_state = [np.random.rand()*2*(self.x_treshold-0.5)-(self.x_treshold-0.5),0.00]
            self.goal_state =(goal_state[0], goal_state[1])
            self.state = (state[0],state[1],(goal_state[0]-state[0]))
    ...
    def hard_reset(self, x, x_dot, x_goal):
        self.state = (x, x_dot, (x_goal - x))
        self.steps_taken = None
        self.steps_beyond_done = None
        self.goal_state = (x_goal, 0.00)
        return self.state
